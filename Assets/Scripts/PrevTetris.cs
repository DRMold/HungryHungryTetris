...
public class PrevTetris : MonoBehaviour {
	...
	bool CheckMove(Vector3 a, Vector3 b, Vector3 c, Vector3 d) {
		...
		if (board[Mathf.RoundToInt(b.x - transform.position.x), Mathf.RoundToInt(b.y - 1 - transform.position.y)] == 1) {
			return false;
		}
		if (board[Mathf.RoundToInt(c.x - transform.position.x), Mathf.RoundToInt(c.y - 1 - transform.position.y)] == 1) {
			return false;
		}
		if (board[Mathf.RoundToInt(d.x - transform.position.x), Mathf.RoundToInt(d.y - 1 - transform.position.y)] == 1) {
			return false;
		}
	...
	bool CheckUserMove(Vector3 a, Vector3 b, Vector3 c, Vector3 d, bool dir) {
		...
		if (dir) { //Left
			if (board[Mathf.RoundToInt(a.x - 1 - transform.position.x), Mathf.RoundToInt(a.y - transform.position.y)] == 1 || board[Mathf.RoundToInt(b.x - 1 - transform.position.x), Mathf.RoundToInt(b.y - transform.position.y)] == 1 || board[Mathf.RoundToInt(c.x - 1 - transform.position.x), Mathf.RoundToInt(c.y - transform.position.y)] == 1 || board[Mathf.RoundToInt(d.x - 1 - transform.position.x), Mathf.RoundToInt(d.y - transform.position.y)] == 1){
				return false;
			}
		} else { //Right
			if(board[Mathf.RoundToInt(a.x + 1 - transform.position.x), Mathf.RoundToInt(a.y - transform.position.y)] == 1 || board[Mathf.RoundToInt(b.x + 1 - transform.position.x), Mathf.RoundToInt(b.y - transform.position.y)] == 1 || board[Mathf.RoundToInt(c.x + 1 - transform.position.x), Mathf.RoundToInt(c.y - transform.position.y)] == 1 || board[Mathf.RoundToInt(d.x + 1 - transform.position.x), Mathf.RoundToInt(d.y - transform.position.y)] == 1){
	...
	bool CheckRotate(Vector3 a, Vector3 b, Vector3 c, Vector3 d){
		if(Mathf.RoundToInt(a.x - transform.position.x) < board.GetLength(0)-1){//Check if block is in board
